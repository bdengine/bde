...
// NewRootCmd creates a new root command for bdeosd. It is called once in the
// main function.
...
func (a appCreator) appExport(
	...
	if height != -1 {
		bdeosApp = app.NewBdeos(logger, db, traceStore, false, map[int64]bool{}, "", uint(1), a.encCfg, appOpts)

		if err := bdeosApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		bdeosApp = app.NewBdeos(logger, db, traceStore, true, map[int64]bool{}, "", uint(1), a.encCfg, appOpts)
	}

	return bdeosApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs)
}
